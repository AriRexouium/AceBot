...
module.exports = class SetStatusCommand extends Command {
  ...
  run (message, args) {
    ...
    this.client.user.setStatus(this.client.provider.get('global', 'clientStatus'))
      .then(
        this.client.user.setActivity(`${this.client.options.commandPrefix}help | ${pluralize('Guild', this.client.guilds.size, true)} | ${pluralize('User', this.client.users.size, true)}${this.client.shard ? ` | Shard ID: ${this.client.shard.id}` : ''}`)
      )
